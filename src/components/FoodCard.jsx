...
import BlurImg from '../assests/blur.jpeg'
import { useNavigate } from 'react-router-dom';

function FoodCard(props) {
  const {id,name , rating , dishes , avg_spend , delivery_time} = props;
  const navigate = useNavigate();
  const takeToInfoPage = () => {
    navigate('/food_info_page', {
      state: {
        dataProp: props
      }
    });
  ...
  return (
    <>
      <div onClick={takeToInfoPage} id={id} className={styles.foodCardHover} style={{display:"flex",flexDirection:"column",padding:"10px 10px 10px 10px" ,borderRadius:"20px",cursor:"pointer"}}>
            <img className='lazy' style={{height: "230px",width: "320px",borderRadius: "15px"}} src={BlurImg} data-src="https://b.zmtcdn.com/data/pictures/chains/3/143/cb4822bd0ecf5e677eea7956c6ebe926_o2_featured_v2.jpg" alt="food" />
